...
backend_set_env() {
  print_banner
  printf "${WHITE} 💻 Configurando variáveis de ambiente (backend - LOCALHOST)...${GRAY_LIGHT}"
  printf "\n\n"

  sleep 2

  # Remove verificações de HTTPS
  backend_url="http://localhost:${backend_port}"
  frontend_url="http://localhost:${frontend_port}"

  sudo su - deploy << EOF
  cat <<[-]EOF > /home/deploy/${instancia_add}/backend/.env
NODE_ENV=development  # Modo desenvolvimento para localhost
